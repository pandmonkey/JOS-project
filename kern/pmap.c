...
static inline void mark_use(size_t i) {
	pages[i].pp_ref = 1;
}
...
// 读取两个连续 register 的值, 并组合成16位的值返回


/**
 * @brief 利用 CMOS calls来检测memory布局, 初始化全局变量 npages npages_basemem方便后续进行内存管理
 * 
 */
static void
...
mem_init(void)
{
	...
	memset(pages, 0, npages * sizeof(struct PageInfo)); // 这里实际上就是对 kernel 的相关数据结构进行初始化
	...
	// or page_insert
	page_init(); // init以后, 分配不用 boot_alloc 用 allocator functions 即可
	...
	// LAB 3: Your code here.
	size_t envs_num = ROUNDUP(NENV * sizeof(struct Env), PGSIZE);
	boot_map_region(kern_pgdir, (uintptr_t)UENVS, envs_num, PADDR(envs), PTE_U | PTE_P);
...
// Does NOT increment the reference
...
page_alloc(int alloc_flags)
{
	...
	// allocate a page
	struct PageInfo *get_page_info = page_free_list;
	page_free_list = get_page_info->pp_link;
	get_page_info->pp_link = NULL;

	// if (alloc_flags & ALLOC_ZERO) fills the entire returned physical page with '\0' bytes
	if (alloc_flags & ALLOC_ZERO) {
		memset(page2kva(get_page_info), 0, PGSIZE);
	}
...
page_free(struct PageInfo *pp)
{
	...
	// check if pp_ref = 0 and pp->pp_link NULL
	assert(pp->pp_ref == 0);
	assert(pp->pp_link == NULL);

	// insert into the free_list
